...
export const ToDayShort = (dt) => dayAsStringShort(new Date(parseInt(dt + '000')).getDay())

// Extract the day from the dt element in an API list e.g. ToDate(1620410400) gives you Friday
export const ToDay = (dt) => dayAsString(new Date(parseInt(dt + '000')).getDay())
...
export const distinctApiDays = list => [...new Set(list.map(item => ToDayShort(item.dt)))]